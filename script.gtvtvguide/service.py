...
try:
    ADDON = xbmcaddon.Addon('script.gtvtvguide')
    if ADDON.getSetting('cache.data.on.xbmc.startup') == 'true':
        Service()
    if ADDON.getSetting('autostart') == "true":
        xbmc.executebuiltin("RunAddon(script.gtvtvguide)")
except source.SourceNotConfiguredException:
    pass  # ignore
except Exception, ex:
    xbmc.log('[script.gtvtvguide] Uncaught exception in service.py: %s' % str(ex) , xbmc.LOGDEBUG)