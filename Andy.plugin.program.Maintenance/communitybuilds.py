...
#CBPATH       =  xbmc.translatePath(os.path.join(USB,'Kodi_Backups',''))
CBPATH       =  xbmc.translatePath(os.path.join(zip,'Kodi_Backups',''))
#CBPATH       =  xbmc.translatePath(os.path.join(USB))
...
def Archive_File(sourcefile, destfile):
    ...
    for_progress = []
    ITEM =[]
    dp.create("[COLOR=white]Maintenance[/COLOR]","Archiving...",'', 'Please Wait')
    for base, dirs, files in os.walk(sourcefile):
        for file in files:
            ITEM.append(file)
    N_ITEM =len(ITEM)
    for base, dirs, files in os.walk(sourcefile):
        for file in files:
            ...
            fn = os.path.join(base, file)
            if not 'temp' in dirs:
                if not 'Andy.plugin.program.Maintenance' in dirs:
                   import time
                   FORCE= '01/01/1980'
                   FILE_DATE=time.strftime('%d/%m/%Y', time.gmtime(os.path.getmtime(fn)))
                   if FILE_DATE > FORCE:
...
def Backup_Option():
    ...
    extras.addDir('','Backup Addons','addons','restore_zip','addon.png','','','Back Up Your Addons')
    extras.addDir('','Backup UserData','addon_data','restore_zip','userdata.png','','','Back Up Your Addon Userdata')
    extras.addDir('','Backup Keymaps','addon_data/keymaps','restore_zip','keymaps.png','','','Back Up Your Keymaps')
    extras.addDir('','Backup Thumbs','addon_data/Thumbnails','restore_zip','thumb.png','','','Back Up Your Thumbnails')
    extras.addDir('','Backup Profiles','addon_data/profiles','restore_zip','Profile.png','','','Back Up Your Profiles')
        
    if os.path.exists(SOURCE):
        extras.addDir('','Backup Source.xml',SOURCE,'restore_backup','XML_SOURCES.png','','','Back Up Your sources.xml')

    if os.path.exists(ADVANCED):
        extras.addDir('','Backup Advancedsettings.xml',ADVANCED,'restore_backup','advancedsettings.jpg','','','Back Up Your advancedsettings.xml')

    if os.path.exists(FAVS):
        extras.addDir('','Backup Favourites.xml',FAVS,'restore_backup','fav.png','','','Back Up Your favourites.xml')
        
    if os.path.exists(RSS):
        extras.addDir('','Backup RssFeeds.xml',RSS,'restore_backup','XML_RSS.png','','','Back Up Your RssFeeds.xml')
        
    if os.path.exists(KEYMAPS):
        extras.addDir('','Backup keyboard.xml',KEYMAPS,'restore_backup','advancedsettings.jpg','','','Back Up Your keyboard.xml')

    if os.path.exists(PROFILES):
        extras.addDir('','Backup profiles.xml',PROFILES,'restore_backup','Profile.png','','','Back Up Your profiles.xml')
                 
    extras.addDir('','Backup Guisettings.xml',GUI,'restore_backup','settings.png','','','Back Up Your guisettings.xml')
     
    extras.addDir('','Open Settings (Set Path)','url','addon_settings','settings.png','','','Set Path')     
...
def Check_Download_Path():
    ...
    if not os.path.exists(zip):
        dialog.ok('[COLOR=white]Maintenance[/COLOR]','Make sure you set your download path.','','')   
...
def Community_Backup():
    guisuccess=1
    Check_Download_Path()
    fullbackuppath = xbmc.translatePath(os.path.join(USB,'Kodi_Backups',''))
    myfullbackup = xbmc.translatePath(os.path.join(USB,'Kodi_Backups','my_full_backup.zip'))
    myfullbackupGUI = xbmc.translatePath(os.path.join(USB,'Kodi_Backups','my_full_backup_GUI_Settings.zip'))

    #fullbackuppath = xbmc.translatePath(os.path.join(USB))
    #myfullbackup = xbmc.translatePath(os.path.join(USB,'my_full_backup.zip'))
    #myfullbackupGUI = xbmc.translatePath(os.path.join(USB,'my_full_backup_GUI_Settings.zip'))
    ...
    #exclude_dirs_full =  ['Andy.plugin.program.Maintenance','script.module.addon.common','Kodi_Backups']
    exclude_dirs_full =  ['Andy.plugin.program.Maintenance','Kodi_Backups']
    #exclude_dirs_full =  ['Andy.plugin.program.Maintenance']
    exclude_files_full = ["xbmc.log","xbmc.old.log","kodi.log","kodi.old.log",'.DS_Store','.setup_complete','XBMCHelper.conf']
    exclude_dirs =  ['plugin.program.kodimasterwizard', 'plugin.repository.shadowcrew','cache', 'system', 'Thumbnails', "peripheral_data",'library','keymaps']
...
def Fix_Special(url):
    dp.create("[COLOR=white]Maintenance[/COLOR]","Renaming paths...",'', 'Please Wait')
...
def Install_Art(path):
    ...
    try:
        ...
        time.sleep(1)
#        Read_Zip(artpack)
        dp.create("[COLOR=white]Maintenance[/COLOR]","Checking ",'', 'Please Wait')
...
def Restore_Backup_XML(name,url,description):
    if 'Backup' in name:
        ...
        if 'guisettings.xml' in description:
            ...
            f.write(TO_READ)
            f.close()  
    dialog.ok("[COLOR=white]Maintenance[/COLOR]", "", 'All Done !','')
#---------------------------------------------------------------------------------------------------
#---------------------------------------------------------------------------------------------------
...
def Restore_Local_Community():
    ...
    if choice == 0:
        ...
        #exclude_dirs_full =  ['Andy.plugin.program.Maintenance','script.module.addon.common','Kodi_Backups']
        #exclude_dirs_full =  ['Andy.plugin.program.Maintenance','Kodi_Backups']
        exclude_dirs_full =  ['Andy.plugin.program.Maintenance','script.module.addon.common']
        exclude_files_full = ["xbmc.log","xbmc.old.log","kodi.log","kodi.old.log",'.DS_Store','.setup_complete','XBMCHelper.conf','guisettings.xml']
        message_header = "Creating full backup of existing build"
        message1 = "Archiving..."
        message2 = ""
        message3 = "Please Wait"
    ...
    if exitfunction == 1:
        ...
        dialog.ok('[COLOR=white]Maintenance[/COLOR]','Step 1 complete. Now please change the skin to','the one this build was designed for. Once done come back','to this addon and restore the guisettings_fix.zip')        
        xbmc.executebuiltin("ActivateWindow(appearancesettings)")
#---------------------------------------------------------------------------------------------------
...
def Restore_Option():
    ...
    extras.addDir('','[COLOR=dodgerblue]Restore Local guisettings file[/COLOR]','url','LocalGUIDialog','settings.png','','','Back Up Your Full System')
    
    if os.path.exists(os.path.join(USB,'addons.zip')):   
        extras.addDir('','Restore Your Addons','addons','restore_zip','addon.png','','','Restore Your Addons')

    if os.path.exists(os.path.join(USB,'addon_data.zip')):   
        extras.addDir('','Restore Your Addon UserData','addon_data','restore_zip','userdata.png','','','Restore Your Addon UserData') 
        
    if os.path.exists(os.path.join(USB,'keymaps.zip')):   
        extras.addDir('','Restore Your Keymaps','addon_data/keymaps','restore_zip','keymaps.png','','','Restore Your Keymaps') 

    if os.path.exists(os.path.join(USB,'profiles.zip')):   
        extras.addDir('','Restore Your Profiles','addon_data/profiles','restore_zip','Profile.png','','','Restore Your Profile') 
 
    if os.path.exists(os.path.join(USB,'Thumbnails.zip')):   
        extras.addDir('','Restore Your Thumbnails','addon_data/keymaps','restore_zip','thumbs.png','','','Restore Your Thumbnails') 
        
    if os.path.exists(os.path.join(USB,'sources.xml')):
        extras.addDir('','Restore Source.xml',SOURCE,'resore_backup','XML_SOURCES.png','','','Restore Your sources.xml')
 
    if os.path.exists(os.path.join(USB,'RssFeeds.xml')):
        extras.addDir('','Restore RssFeeds.xml',RSS,'resore_backup','XML_RSS.png','','','Restore Your RssFeeds.xml') 
          
    if os.path.exists(os.path.join(USB,'favourites.xml')):
        extras.addDir('','Restore Favourites.xml',FAVS,'resore_backup','fav.png','','','Restore Your favourites.xml')       
        
    if os.path.exists(os.path.join(USB,'advancedsettings.xml')):
        extras.addDir('','Restore Advancedsettings.xml',ADVANCED,'resore_backup','advancedsettings.jpg','','','Restore Your advancedsettings.xml')        

    if os.path.exists(os.path.join(USB,'keyboard.xml')):
        extras.addDir('','Restore keyboard.xml',KEYMAPS,'resore_backup','settings.png','','','Restore Your keyboard.xml') 
    
    if os.path.exists(os.path.join(USB,'profiles.xml')):
        extras.addDir('','Restore profiles.xml',GUI,'resore_backup','Profile.png','','','Restore Your profile.xml')

    if os.path.exists(os.path.join(USB,'guisettings.xml')):
        extras.addDir('','Restore Guisettings.xml',GUI,'resore_backup','settings.png','','','Restore Your guisettings.xml')
...
def Restore_Zip_File(url):
    ...
    if 'Backup' in name:
        Delete_Packages() 
        dp.create("[COLOR=white]Maintenance[/COLOR]","Backing Up",'', 'Please Wait')
        ...
        zipobj.close()
        dp.close()
        dialog.ok("[COLOR=white]Maintenance[/COLOR]", "You Are Now Backed Up", '','')   
    else:
        dp.create("[COLOR=white]Maintenance[/COLOR]","Checking ",'', 'Please Wait')
        dp.update(0,"", "Extracting Zip Please Wait")
        ...
        if 'Backup' in name:
            #extras.Kill_XBMC()
            dialog.ok("Builds - Install Complete", 'To ensure the skin settings are set correctly XBMC will now', 'close. If XBMC doesn\'t close please force close (pull power', 'or force close in your OS - [COLOR=lime]DO NOT exit via Kodi menu[/COLOR])')
        else:
            dialog.ok("[COLOR=white]Maintenance[/COLOR]", "You Are Now Restored", '','')        
#---------------------------------------------------------------------------------------------------
